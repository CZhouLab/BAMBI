...
def Jobstatus_Check(resubmit):
    ...
    # pvalue_type = "padj"  # ["padj", "pvalue"]
    # pvalue_threshold = str(0.05)
    # linc_min_FPKM_expression = str(0.1)
    # PC_min_FPKM_expression = str(1.0)
    # Circ_min_FPKM_expression = str(0.1)



    # DE_output_folder_path = output_directory + "/" + DE_type + "_DE_output"
    # #neccessary only for heatmap rebuild
    # linc_padj_path = DE_output_folder_path + "/summary_step1.txt.ReadCount.lincRNA.DE.output"
    # PC_padj_path = DE_output_folder_path + "/summary_step1.txt.ReadCount.ProteinCoding.DE.output"
    # linc_FPKM_path = DE_output_folder_path + "/summary_step1.txt.FPKM.lincRNA.DEAoutput"
    # PC_FPKM_path = DE_output_folder_path + "/summary_step1.txt.FPKM.ProteinCoding.DEAoutput"
    # Circ_padj_path = DE_output_folder_path + "/GSE171936.txt.edgeR.output"
    # Circ_FPKM_path = DE_output_folder_path + "/GSE171936_normalize.DEC.txt"


    report_path_record_list = []

    current_partition = 0
    # for current_partition in range(len(folder_list)):
