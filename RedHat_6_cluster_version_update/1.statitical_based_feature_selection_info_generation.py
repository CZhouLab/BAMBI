...
def statitical_based_feature_selection_info_generation(biomarker_target_gene_type="protein_coding", RNASeq_FPKM_table_path=None,
    ...
    for gene_type in gene_type_list:
        ...
        if not os.path.isdir(tem_output_directory):
        ...
        if datatype == "original_RNA_Seq":
            ...
            ##it may revised when we apply the DE on both PC & lncRNA
            standard_ReadCount_whole_df = standard_ReadCount_df.copy()
            update_standard_ReadCount_whole_df = standard_ReadCount_whole_df.reset_index()
            update_standard_ReadCount_whole_df.index = [biomarker_target_gene_type] * len(standard_ReadCount_whole_df)
            update_standard_ReadCount_whole_df.index.name = "GeneType"
            update_standard_ReadCount_whole_df.to_csv(output_directory + "/summary_step1.txt.ReadCount", sep="\t")
