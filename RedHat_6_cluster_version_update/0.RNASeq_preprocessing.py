...
def RNASeq_preprocessing(biomarker_target_gene_type = "protein_coding", sequence_type="Paired", annotation_file="LncBook_Version2.0_all", inputCSV="./0.RNASeq_preprocessing_input_sample_Paired-End.csv", HPC_parallel = False):
    ...
    def job_submission(sample_name, strandness, src_path, output_directory, Known_splice_site, GTF_file, ExonLength_file, sequence_type, R1_input="NA", R2_input="NA", unpaired_input = "NA", HPC_parallel = False):
        ...
        if HPC_parallel:
            ...
            bash_string_1_R = bash_string_1.format(pyscript_path=pyscript_path, sample_name=sample_name,
                                                   R1_input=R1_input, R2_input=R2_input, unpaired_input=unpaired_input,
                                                   strandness=strandness, directory=directory,
                                                   Known_splice_site=Known_splice_site, GTF_file=GTF_file,
            ...
            os.chdir(save_directory)
            run_Pipeline_tem = subprocess.Popen(['python', pyscript_path,
                                                 ...
                                                 '--directory', directory,
                                                 '--Known_splice_site', Known_splice_site,
                                                 '--GTF_file', GTF_file,
                                                 '--ExonLength_file', ExonLength_file,
    ...
    if from_PreProcessing:
        ...
        summary_step1_df = summary_step1_df[["sample", "HTSEQ_count_path", "Gene_FPKM", "Label"]]
        # print(summary_step1_df)
    ...
    if from_FPKM_ReadCount_generation:
        ...
        if "microarray" not in gene_type_list:
            Pipeline_step_2_1_para_1 = src_path + '/Expression_Matrix_RNAType_updated_py2_ver.py'
